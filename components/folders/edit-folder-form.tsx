...
export const EditFolderModalForm = ({
  folder,
  ...
  const form = useForm<z.infer<typeof ZEditFolderSchema>>({
    resolver: zodResolver(ZEditFolderSchema),
    defaultValues: {
      name: folder.name,
  ...
  async function onSubmit(values: z.infer<typeof ZEditFolderSchema>) {
    try {
      ...
      toast.success("Folder details updated successfully");
    } catch (error) {
      console.log(error);
      toast.error("Failed to update folder details. Please try again.");
    } finally {
      form.reset();
